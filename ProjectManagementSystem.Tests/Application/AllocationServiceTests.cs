...
public class AllocationServiceTests
{
    ...
    public async Task CreateAsync_ThrowsWhenEmployeeNotOnManagerTeam()
    {
        ...
        _employeeRepo.Setup(r => r.IsOnManagerTeamAsync(10, 1)).ReturnsAsync(false);

        await Assert.ThrowsAsync<ForbiddenAppException>(
            () => _sut.CreateAsync(CreateValidAllocationDto(), managerUserId: 10));
    }
    ...
    {
        SetupValidEmployeeAndProject();
        _employeeRepo.Setup(r => r.IsOnManagerTeamAsync(10, 1)).ReturnsAsync(true);
        _allocationRepo.Setup(r => r.GetByEmployeeIdAsync(1)).ReturnsAsync(new[]
        {
            new AllocationDto
            {
                Id = 99,
        ...
        await Assert.ThrowsAsync<BusinessRuleException>(
            () => _sut.CreateAsync(CreateValidAllocationDto(utilisation: 30), managerUserId: 10));
    }
    ...
    {
        SetupValidEmployeeAndProject();
        _projectRepo.Setup(r => r.GetByIdAsync(201))
    ...
    {
        _allocationRepo.Setup(r => r.GetByIdAsync(5)).ReturnsAsync(new AllocationDto
        {
            Id = 5,
            FromDate = new DateOnly(2026, 6, 1),
            ToDate = new DateOnly(2026, 9, 30),
    ...
    [Fact]
    public async Task GetAllActiveAsync_ReturnsActiveAllocations()
    {
        var allocations = new[] { new AllocationDto { Id = 1, IsActive = true } };
        _allocationRepo.Setup(r => r.GetAllActiveAsync()).ReturnsAsync(allocations);
    ...
    {
        _employeeRepo.Setup(r => r.GetByIdAsync(1))
            .ReturnsAsync(new EmployeeDto { Id = 1, IsActive = true });
        _employeeRepo.Setup(r => r.IsAllocatableResourceAsync(1)).ReturnsAsync(true);
        _projectRepo.Setup(r => r.GetByIdAsync(201))
            .ReturnsAsync(new ProjectDto { Id = 201, Status = "Active" });
        _allocationRepo.Setup(r => r.GetByEmployeeIdAsync(1)).ReturnsAsync(Array.Empty<AllocationDto>());
    }
