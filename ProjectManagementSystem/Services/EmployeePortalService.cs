    ...
    ISystemConfigRepository configRepo) : IEmployeePortalService
{
    ...
    public async Task<TimesheetDto> SubmitTimesheetAsync(int userId, SubmitEmployeeTimesheetDto dto)
    {
        ...
        if (weekAllocations.Count == 0)
            throw new InvalidOperationException("You have no active project allocations for this week.");
        ...
        if (createDto.Entries.Count == 0)
            throw new InvalidOperationException("At least one project must have hours greater than zero.");
    ...
    {
        if (entries.Count == 0)
            throw new InvalidOperationException("At least one project entry is required.");

        if (entries.Select(p => p.ProjectId).Distinct().Count() != entries.Count)
            throw new InvalidOperationException("Duplicate project entries are not allowed in one timesheet.");
        ...
        foreach (var entry in entries)
        {
            ...
            if (!allowedProjectIds.Contains(entry.ProjectId))
            {
                throw new InvalidOperationException(
                    $"You are not allocated to project ID {entry.ProjectId} during this week.");
            }

            if (entry.Hours > maxByProject[entry.ProjectId])
            {
                throw new InvalidOperationException(
                    $"Hours for project ID {entry.ProjectId} exceed the allowed maximum " +
                    $"({maxByProject[entry.ProjectId]} hrs based on your allocation).");
            }
        ...
        int employeeId, DateTime weekMonday, int maxWeeklyHours)
    {
        ...
        return (await allocationRepo.GetByEmployeeIdAsync(employeeId))
            ...
            .Select(a => new EmployeeWeekAllocationDto
            {
                ...
                MaxHours = Math.Round(a.UtilisationPercent * maxWeeklyHours / 100m, 2),
                FromDate = a.FromDate,
                ToDate = a.ToDate
            })
            .ToList();
