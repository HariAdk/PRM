...
using ProjectManagementSystem.Services.AI;
    ...
    ILogger<AiService> logger) : IAiService
{
        ...
        if (providerFactory.IsConfigured(config))
        {
            try
            {
                ...
                if (parsed.Count > 0)
                {
                    ...
                    return new AISkillMatchResultDto { Matches = parsed };
                }
        ...
        Core.DTOs.Project.ProjectDto project)
    {
        ...
        var allocationDetails = allocations
            ...
            .Select(a =>
            {
                var expected = Math.Round(a.UtilisationPercent * maxWeeklyHours / 100m, 0);
